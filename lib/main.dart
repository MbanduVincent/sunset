...
Future Ticketing() async {
  ...
  List<Map<String, dynamic>> data =
      List<Map<String, dynamic>>.from(resulting.map((row) {
    return {
      "id": row[0],
      "name": row[1],
      "age": row[2],
...
class _TicketingScreenState extends State<TicketingScreen> {
  ...
  Future<void> _editTicket(int id) async {
    ...
    final ageController =
        TextEditingController(text: selectedTicket['age'].toString());
    final addressController =
        TextEditingController(text: selectedTicket['address']);
    final salaryController =
        TextEditingController(text: selectedTicket['salary'].toString());
  ...
  Future<void> updateTicket(Map<String, dynamic> ticketData) async {
    ...
    await connect.query(
        'UPDATE customers SET name="$name", age=$age, address="$address", salary=$salary WHERE id=$id');
  }
